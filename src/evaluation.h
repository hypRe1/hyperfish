...
// Evaluation function adapted from the PeSTO chess engine
// Uses piece-square tables and tapered evaluation to quickly evaluate positions
...
void init_tables() {
    for (int piece = P; piece <= K; piece++) {
        for (int square = 0; square < 64; square++) {
            ...
            mg_table[piece+6][square] = mg_value[piece] + mg_pesto_table[piece][flip(square)];
            eg_table[piece+6][square] = eg_value[piece] + eg_pesto_table[piece][flip(square)];
        }
    }
}

static inline int evaluate(struct Board* board) {
    int mgScore = 0;
    int egScore = 0;
    int gamePhase = 0;
